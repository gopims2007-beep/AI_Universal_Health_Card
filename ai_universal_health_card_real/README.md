...
### Render + Supabase
The backend is configured for Supabase PostgreSQL through SQLAlchemy. In the
Render service environment variables, set:

```text
DATABASE_URL=postgresql+psycopg://postgres:<URL-ENCODED-PASSWORD>@db.czgwamixedriwvitlkse.supabase.co:5432/postgres
...
`JWT_SECRET` is configured in `render.yaml` to be generated by Render. Keep
the database password, service-role key, and JWT secret private. Create a
private Supabase Storage bucket named `medical-reports` before uploading files.
If the direct Supabase host is
not reachable from the Render service, use the Supabase Session Pooler
connection string from the Supabase Connect dialog instead.

The application creates its SQL tables automatically on startup. Uploaded
medical files are uploaded to the private Supabase Storage bucket; the
database stores their metadata. The backend uses the service-role key only on
the server, so never put it in frontend JavaScript.

On startup, Row Level Security is enabled for the application tables. The
backend uses its server-side database connection, while direct public access
through Supabase's API remains blocked unless explicit policies are added.
