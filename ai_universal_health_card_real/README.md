...
the database password and JWT secret private. If the direct Supabase host is
...
medical files remain in the configured `UPLOAD_DIR`; the database stores
their metadata. Render's local filesystem is ephemeral, so use persistent
object storage before relying on uploaded files in production.
